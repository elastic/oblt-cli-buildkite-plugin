...
| `username`                   | `string` | repository name    | The oblt-cli username. Defaults to the repository name derived from `BUILDKITE_REPO` (e.g. `my-project` from `git@github.com:acme-inc/my-project.git`), falling back to `obltmachine` if `BUILDKITE_REPO` is not set. |
